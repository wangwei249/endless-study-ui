<template>
  <div >
    ...
    <el-dialog :title="titlePractise" 
               :visible.sync="openPractise2" width="850px" 
               :show-close="false" 
               @close="closePractise" 
        ...
        <el-row :gutter="24"> 
          ...
          <el-col :span="18" align="middle"> 
             <el-card  shadow="always" body-style="{ padding: '10px' }">
              <span :style="wordStyle" v-if="this.practiseType == 'english'"> 
                  {{this.currentWord.word}} 
              </span>  
              <span :style="wordStyle" v-if="this.practiseType == 'means'">  
                  <span style="margin-left: 2px" v-for="(item, index)  in this.currentWord.meansList" >   
                      <span v-if="index != item.length-1" ><font style="color:green;font-size:18px;">{{item.posName}}</font> &nbsp;&nbsp;&nbsp;{{item.means}}  </br></br> </span> 
                      <span v-if="index == item.length-1" ><font style="color:green;font-size:16px;">{{item.posName}}</font> &nbsp;&nbsp;&nbsp;{{item.means}} </span> 
                  </span>   
              </span>  
             </el-card>  
          </el-col>
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            ...
            <div v-for="(item, index) in this.suggestWords"  style="display: flex;"  v-if="this.practiseType == 'english'"> 
            ...
            <div v-for="(item, index) in this.suggestWords"  style="display: flex;"  v-if="this.practiseType == 'means'">  
              <el-card class="box-card"  @click.native="checkAnswer(item,index)" >  
                  {{item.word}}
              </el-card> 
            </div>
...
export default {
  ...
  methods: {
      ...
      if(this.practiseWordArray.length<=0){ 
          await listNewWordsRandom(this.queryParams).then(response => {
              if(response.rows.length<=0){
                  ...
                  //alert(JSON.stringify(this.practiseWordArray.pop()));
                  this.currentWord = JSON.parse(JSON.stringify(this.practiseWordArray.pop()));
    ...
    async checkAnswer(wordClick,index){
        ...
        if(wordClick.word == this.currentWord.word){
            ...
            if(this.practiseType == 'english'){
                this.wordStyle = "margin-left: 10px;color:green;font-size:68px;";
            }else if(this.practiseType == 'means'){
                this.wordStyle = "margin-left: 10px;color:green;font-size:25px;";
            }else if(this.practiseType == 'lisen'){
              
            }else if(this.practiseType == 'spell'){
            ...
            if(this.practiseType == 'english'){
                ...
                this.wordStyle = "margin-left: 10px;color:red;font-size:25px;";
            }else if(this.practiseType == 'lisen'){
    ...
    closePractise(){ 
     //alert(this.$refs.PractiseNewWord);
      ...
      this.$emit("getList");
      this.$emit("getNewWordCollect");
      this.$emit("listNewWordChallenge");
      //this.$refs.PractiseNewWord.closePractise();
      if(this.clock){  
        window.clearInterval(this.clock);
      }  
      this.openPractise2 = false;
    },
  ...
  watch: {
    ...
    practiseType: {
      ...
      handler(val) {
        alert(val);
        this.practiseType = val;
        if(val == 'english'){
           this.wordStyle = "margin: 20px;color:black;font-size:58px;font-family:Arial;";
        }else if(val == 'means'){
           this.wordStyle = "margin: 20px;color:black;font-size:20px;font-family:Arial;";
        }else if(val == 'lisen'){
